...
const {
  loggers: { logger },
  createJWT,
  getPrivateKeyForWorkstation
} = require("csg-cwf-shared-components");
...
const createLoginRequest = async auth => {
  ...
  try {
    ...
    logger.debug(
      `There was a failure issuing a request to the citco SSO url: ${options}`
    );
    throw e;
  }
...
const verifySAMLFormSubmition = (response, cookieJar, options) => {
  const JWTCookie = cookieJar
    .toJSON()
    .cookies.find(cookie => cookie.key === "JWT");
  if (!JWTCookie) {
...
const validateSAMLResponse = (responsetext, cookieJar) => {
  ...
  const SMSessionCookie = cookieJar
    .toJSON()
    .cookies.find(cookie => cookie.key === "SMSESSION");
  if (!SMSessionCookie) {
    throw new Error(
      `Siteminder did not return a SMSESSION cookie, please try again.`
    );
  }
...
const createSamlRequest = async (loginRequest, auth) => {
  ...
  const dom = cheerio.load(response.data, { decodeEntities: false });
  const SMSESSION = cookieJar
    .toJSON()
    .cookies.find(cookie => cookie.key === "SMSESSION").value;
  const payload = buildPayload(dom, auth);
...
const findJWTCookie = cookieJar => {
  ...
  const JWT = cookies.filter(
    cookie => cookie.key === "JWT" || cookie.key === "SMSESSION"
  );
  if (!JWT) {
    throw new Error(
      `No JWT cookie could be found in the cookieJar: ${JSON.stringify(
        cookieJar.toJSON()
      )}`
    );
  }
...
const getJWT = async ({ username, password, env }) => {
  try {
    let JWT;
    if (env === "workstation") {
      const exp = moment()
        .add(5, "minutes")
        .unix();
      JWT = createJWT(getPrivateKeyForWorkstation(), exp, username);
    } else {
