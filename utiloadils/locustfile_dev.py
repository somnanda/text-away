...
class TransactionNav(TaskSet):
    ...
    def get_nav(self):
        ...
        jwt = get_jwt(self)
        headers = {
            'waterfallId': '6f732d5e-4ca0-45eb-909d-517b6e15c3ab',
            'Cookie': 'JWT=' + jwt
            }

        with self.client.get("/core/transactions/nav/6f732d5e-4ca0-45eb-909d-517b6e15c3ab", verify=False, headers=headers, catch_response=True) as response:
            status = str(response.status_code)
            reason = response.reason
...
class TransactionContributions(TaskSet):
    ...
    def get_contributions(self):
        ...
        with self.client.get("/core/transactions/contribution/6f732d5e-4ca0-45eb-909d-517b6e15c3ab", verify=False, headers=headers, catch_response=True) as response:
            status = str(response.status_code)
            reason = response.reason
            if response.status_code == 200:
                response.success()
...
class TransactionDistributions(TaskSet):
    ...
    def get_distributions(self):
        ...
        with self.client.get("/core/transactions/distribution/6f732d5e-4ca0-45eb-909d-517b6e15c3ab", verify=False, headers=headers, catch_response=True) as response:
            status = str(response.status_code)
            reason = response.reason
            if response.status_code == 200:
                response.success()
...
class WaterfallClient(HttpLocust):
    ...
    host = "https://waterfall.dev.srvs.int.csgdev01.citcosvc.com"
    min_wait = 60000
    max_wait = 90000
